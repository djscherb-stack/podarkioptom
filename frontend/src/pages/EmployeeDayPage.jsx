...
export default function EmployeeDayPage() {
  ...
  useEffect(() => {
    if (selectedDate) {
      setLoading(true)
      setError(null)
      apiFetch(`${API}/employee-day/${selectedDate}`)
        .then(setData)
  ...
  const handleRefresh = () => {
    apiFetch(`${API}/refresh`, { method: 'GET' }).then(() => {
      apiFetch(`${API}/employee-day/${selectedDate}`).then(setData)
    }).catch(e => setError(e.message))
  }
  ...
  return (
    <div className="page day-page">
      ...
      {!loading && (
        <>
          <ProductionBlock prodName="ЧАЙ" prodData={productions.ЧАЙ} expandedKey={expandedKey} onToggle={setExpandedKey} />
          <ProductionBlock prodName="ГРАВИРОВКА" prodData={productions.ГРАВИРОВКА} expandedKey={expandedKey} onToggle={setExpandedKey} />
          <ProductionBlock prodName="ЛЮМИНАРК" prodData={productions.ЛЮМИНАРК} expandedKey={expandedKey} onToggle={setExpandedKey} />
